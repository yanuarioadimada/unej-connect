...
if (isset($_SESSION["authenticated"]) && $_SESSION["authenticated"][1] == "false") {
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["readySubmit"])) {
        ...
        if (empty($_POST["start"])) {
            $errors_ .= Util::displayAlertV1("Tolong pilih tanggal mulai", "info");
        }
        if (empty($_POST["end"])) {
            $errors_ .= Util::displayAlertV1("Tolong pilih tanggal selesai", "info");
        }
        if (!DateTime::createFromFormat('Y-m-d', $_POST["start"])) {
            $errors_ .= Util::displayAlertV1("Tanggal mulai tidak sesuai.", "info");
        }
        if (!DateTime::createFromFormat('Y-m-d', $_POST["end"])) {
            $errors_ .= Util::displayAlertV1("Tanggal selesai tidak sesuai.", "info");
        }
        if (empty($_POST["type"])) {
            $errors_ .= Util::displayAlertV1("Tolong pilih tipe gedung", "info");
        }
        if (empty($_POST["adults"])) {
            $errors_ .= Util::displayAlertV1("Tolong isikan berapa jumlah panitia", "info");
        }
        ...
        try {
            ...
            if ($endDate <= $startDate) {
                $errors_ .= Util::displayAlertV1("Tanggal selesai tidak bisa sama dengan tanggal mulai", "info");
            }
        } catch (Exception $e) {
            $errors_ .= Util::displayAlertV1("tipe data tidak sesuai", "info");
        }
